...
const Dashboard: React.FC<DashboardProps> = ({ 
  ...
  // Разделение по магическому разделителю '---'
  ...
  return (
    <div className="relative z-10 p-4 max-w-lg mx-auto h-screen flex flex-col justify-between overflow-hidden animate-fadeIn">
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        {/* 1. HEADER */}
        <header className="flex justify-between items-start mb-2 border-b border-white/5 pb-2 shrink-0">
            <div>
        ...
        <section className="mb-2 shrink-0">
            <div className="relative w-full h-[55px] bg-gradient-to-r from-[#1a1a1a] to-black rounded-xl border border-gold/15 flex flex-row items-center p-3 gap-3 overflow-hidden shadow-lg">
        ...
        <section className="relative flex-[1.5] min-h-0 flex flex-col mb-3">
            <div className="relative flex-1 bg-black/30 border border-white/5 rounded-2xl flex flex-col shadow-2xl backdrop-blur-sm overflow-hidden">
                <div className="p-4 flex-1 flex flex-col overflow-hidden">
                    <h2 className="font-cinzel text-center text-gold/40 text-[7px] font-bold tracking-[0.6em] mb-3 uppercase">✧ ПРОРОЧЕСТВО ✧</h2>
                    
                    <div className="relative flex-1 flex flex-col overflow-hidden">
                        {/* THE HOOK (Unlocked Intro) */}
                        <div className="relative z-10 shrink-0">
                           <p className="text-white font-cinzel text-[13px] leading-relaxed text-center italic px-1 mb-2">
                                {introText}
                            </p>
                        </div>
                        ...
                        <div className="relative flex-1 overflow-hidden">
                            <div className={`transition-all duration-1000 h-full ${isLocked ? 'blur-xl opacity-70' : 'blur-0 opacity-100 overflow-y-auto'}`}>
                                <p className="text-gray-300 font-lato text-xs leading-relaxed text-center whitespace-pre-wrap px-4 italic pb-6 pt-2">
                                    {mainText || "Звезды готовят финальный ответ..."}
                                </p>
                            </div>
                            ...
                            {isLocked && (
                                <div className="absolute inset-0 flex flex-col items-center justify-center z-20">
                                    ...
                                    <div className="relative z-30 w-full px-6 flex flex-col items-center gap-5 mt-4">
                                        <button 
                                            onClick={handleUnlockClick}
                                            className="group relative w-full max-w-[220px] bg-gradient-to-b from-gold to-[#B8860B] text-black font-cinzel font-bold py-3.5 rounded-full shadow-[0_15px_35px_rgba(0,0,0,0.9)] active:scale-95 transition-all flex flex-col items-center overflow-hidden border border-white/20"
                                        >
                                            <div className="absolute inset-0 bg-white/20 translate-x-[-100%] group-hover:translate-x-[100%] transition-transform duration-1000"></div>
                                            <span className="text-[9px] tracking-[0.15em] uppercase">Раскрыть Откровение</span>
                                            <span className="text-[6px] mt-0.5 opacity-60 uppercase font-lato font-normal tracking-[0.2em]">Узнать продолжение</span>
                                        </button>
                                        ...
                                        <div className="flex flex-col items-center gap-1 opacity-90">
                                            ...
                                            <p className="text-[9px] text-white/70 uppercase tracking-[0.2em] font-lato text-center font-bold">
      ...
      <div className="shrink-0 mb-2">
        <div className="grid grid-cols-3 gap-2 mb-3">
            <StatCard label="Карма" value={prediction.karma} color="from-purple-600/15 to-purple-900/25" borderColor="border-purple-500/10" textColor="text-purple-300" />
            <StatCard label="Удача" value={prediction.luck} color="from-gold/15 to-yellow-900/25" borderColor="border-gold/10" textColor="text-gold" />
            <StatCard label="Любовь" value={prediction.love} color="from-pink-600/15 to-pink-900/25" borderColor="border-pink-500/10" textColor="text-pink-300" />
        </div>

        <button onClick={onReset} className="w-full text-center text-[6px] text-white/10 uppercase tracking-[0.5em] font-cinzel py-1">
            — Перезагрузка —
        </button>
      </div>
                  ...
                  <div className="space-y-3">
                      ...
                      >
                          <div className="text-left">
                              <p className="text-[10px] font-cinzel text-white tracking-widest uppercase">Сегодняшний Путь</p>
                              <p className="text-[8px] text-gray-500 uppercase mt-0.5">Доступ на 24 часа</p>
                          </div>
...
const StatCard: React.FC<{ label: string; value: number; color: string; borderColor: string; textColor: string }> = ({ label, value, color, borderColor, textColor }) => (
    <div className={`bg-gradient-to-b ${color} ${borderColor} border rounded-xl p-2 flex flex-col items-center justify-center gap-0.5 shadow-md`}>
        <span className="text-[7px] uppercase text-white/30 font-cinzel tracking-wider">{label}</span>
